...
(function(){
	...
	dojo.declare("dojox.charting.Chart2D", null, {
		...
		fullGeometry: function(){
            ...
            if(offsets.l > 150){offsets.l = 150}; // gets too wide for some reason in some cases SOFA - was 60, now 150
		...
		fullRender: function(){
		    ...
		    for(var i in axes){
			    ...
			    if(axis.opt.title){
				    ...
				    if(axis.vertical){
					    rotate = 270;
					    y = y_middle;
					    x = this.yTitleOffset; // distance to left of axis (needs to clear labels) SOFA - was always 45
				    }else{
					    x = x_middle;
					    y = dim.height - 2;
				    }
				    ...
				    if(rotate){//alert("offsets: " + offsets.l + ", x: " + x + ", x_middle: " + x_middle);
					    ...
					    elem.setTransform(m.translate(0, y - y_middle -3))
				    }
			    }
		    };
		    ...
		    var elemN = this.surface.createText({
			    x: offsets.l+5,
			    y: offsets.t+11,
			    text: nChart,
			    align: 'left'
...
(function(){
	...
	dojo.declare("dojox.charting.action2d.Tooltip", dojox.charting.action2d.Base, {
		...
		constructor: function(chart, plot, kwArgs){
			...
			this.tooltipBorderColour = kwArgs.tooltipBorderColour
		...
		process: function(o){
			...
			showChartingTooltip(this.text(o), this.aroundRect, position, "center", this.tooltipBorderColour, this.connectorStyle);
		}
	});
	...
	var MasterTooltip = dojo.declare(dijit._MasterTooltip, {
		...
		show: function(/*String*/ innerHTML, /*DomNode*/ aroundNode, /*String[]?*/ position, /*String*/alignment, /*String*/tooltipBorderColour, /*String*/connectorStyle){ // modification of existing line
			...
			// show it
			dojo.style(this.domNode, "opacity", 0);
            dojo.style(this.containerNode, "borderColor", tooltipBorderColour); // new line
			this.fadeIn.play();
			this.isShowingNow = true;
			this.aroundNode = aroundNode;
		},
...
dojo.provide("dojox.charting.plot2d.Boxplot");
...
(function(){
	...
	dojo.declare("dojox.charting.plot2d.Boxplot", dojox.charting.plot2d.Base, {
		defaultParams: {
		    hAxis: "x",
		    vAxis: "y",
		    gap:    15,
		...
		calculateAxes: function(dim){
	       ...
	       render: function(dim, offsets){
		        ...
		        for(var i = this.series.length - 1; i >= 0; --i){
			        ...
			        var stroke = {color: run.stroke, width: borderWidth};
			        ...
			        var rect = {x: center - width / 2,
			            ...
			            var whiskerStroke = {color: run.stroke, width: 2};
                        var endLineWidth = width*0.5;
                        lowerLine = s.createRect({x: center - endLineWidth/2,
                                                  y: yoff - (lw > baseline ? vt(lw) : baselineHeight),
                                                  width: endLineWidth,
                                                  height: 1}).setStroke(whiskerStroke);
                        ...
                        upperLine = s.createRect({x: center - endLineWidth/2,
                                                  y: yoff - (uw > baseline ? vt(uw) : baselineHeight),
                                                  width: endLineWidth,
                                                  height: 1}).setStroke(whiskerStroke);
			            var lowerConnector = s.createLine({x1: center, y1: yoff - (lq > baseline ? vt(lq) : baselineHeight),
						           x2: center, y2: yoff - (lw > baseline ? vt(lw) : baselineHeight)}).setStroke(stroke);
			            var upperConnector = s.createLine({x1: center, y1: yoff - (uw > baseline ? vt(uw) : baselineHeight),
			            ...
			            if(this.opt.markers){
				            ...
				            dojo.forEach(outliers, function(y, k){
					                ...
					                var path = "M" + xp + " " + yp + " " + "m-3,0 c0,-4 6,-4 6,0 m-6,0 c0,4 6,4 6,0";
					                markers[k] = s.createPath(path).setStroke(stroke).setFill(fill);
				                },
                                this);
				            if(events){
				                dojo.forEach(markers, function(s, k){
					                ...
					                o.y = outliers_rounded[k];  // tooltip
					                this._connectEvents(o);
					            }, this);
				            }
			            }
